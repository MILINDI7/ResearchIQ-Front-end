...
import { Heart, MessageCircle, Share2, Bookmark, TrendingUp, Users as UsersIcon, Sparkles, FileText, Calendar, Award, Link2 } from 'lucide-react';
...
export function FeedPage() {
  ...
  const { research, researchers, user, likeResearch } = useApp();
  ...
  const activeCollaborations = [
    { name: 'Dr. Wei Chen', topic: 'Climate modeling', efficiency: '87%' },
    { name: 'Dr. Priya Sharma', topic: 'Statistical analysis', efficiency: '92%' },
  ];
      ...
      <Dialog open={!!viewDetails} onOpenChange={() => setViewDetails(null)}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          {detailsResearch && detailsAuthor && (
            <div className="space-y-6">
              {/* Cover Image */}
              {detailsResearch.coverImage && (
                <div className="relative h-64 bg-gray-100 rounded-lg overflow-hidden -mt-2">
                  <img
                    src={detailsResearch.coverImage}
                    alt={detailsResearch.title}
                    className="w-full h-full object-cover"
                  />
                </div>
              )}
